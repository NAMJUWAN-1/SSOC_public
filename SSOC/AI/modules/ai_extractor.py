...
## 🚨 1. 일정 정보 부재 시 처리 (CRITICAL RULE - 가장 우선순위 높음)
**본문에 구체적인 날짜, 요일, 상대적 표현(오늘/어제 등), 시각 정보가 "전혀" 존재하지 않는 경우:**
* 임의의 시간을 추측하거나 생성(Hallucination)하지 마세요.
* **반드시** `start_at`과 `end_at` 모두를 **게시글 생성일자({posted_at})**와 소수점까지 동일하게 설정하세요.
* 이것은 해당 공지를 특정 시점의 '사건(Milestone)'으로 기록하기 위함입니다.
